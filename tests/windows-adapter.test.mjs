...
function runAsync(adapter, args = [], environment = {}) {
  return new Promise((resolve, reject) => {
    const child = spawn(process.env.ComSpec, ["/d", "/c", adapter, ...args], {
      env: { ...process.env, ...environment },
    });
    let stdout = "";
    let stderr = "";
...
windowsTest("the Windows Host adapter supports concurrent first-use installation", async (context) => {
  const fixture = await createPluginFixture();
  context.after(() => rm(fixture.root, { recursive: true, force: true }));
  const environment = { PROJECT_GRAPH_PLUGIN_DATA: fixture.dataRoot };
  const payloadRoot = join(fixture.root, "payloads/win32-x64/cli");
...
windowsTest("the Windows Host adapter repairs an invalid cached runtime", async (context) => {
  const fixture = await createPluginFixture();
  context.after(() => rm(fixture.root, { recursive: true, force: true }));
  const runtimeRoot = join(fixture.dataRoot, "runtime/0.1.2/win32-x64");
  ...
  const packageLock = await readFile(join(fixture.root, "payloads/win32-x64/cli/package-lock.json"));
