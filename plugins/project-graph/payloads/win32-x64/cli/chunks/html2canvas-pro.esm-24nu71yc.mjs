//#region ../node_modules/.pnpm/html2canvas-pro@2.0.2/node_modules/html2canvas-pro/dist/html2canvas-pro.esm.js
/*!
...
var Tokenizer = class {
	...
	consumeUnicodeRangeToken() {
		...
		if (questionMarks) return {
			type: 30,
			start: parseInt(fromCodePoint$1(...digits.map((digit) => digit === QUESTION_MARK ? ZERO : digit)), 16),
			end: parseInt(fromCodePoint$1(...digits.map((digit) => digit === QUESTION_MARK ? F : digit)), 16)
		};
	...
	if (!isRelativeTransform(tokens)) {
		...
		const extractComponent = (color, token) => {
			...
			if (token.type === 18) {
				...
				if (token.name === "calc") {
					const expression = parseCalc(args).replace(/r|x/, color[0].toString()).replace(/g|y/, color[1].toString()).replace(/b|z/, color[2].toString());
	...
	parse: (_context, tokens) => {
		return parseFunctionArgs(tokens).map((values) => {
			return values.map((value) => {
				if (isCalcFunction(value)) return evaluateCalcToLengthPercentage(value, 0);
...
var imageRendering = {
	...
	parse: (_context, value) => {
		switch (value.toLowerCase()) {
			...
			case "-moz-crisp-edges": return IMAGE_RENDERING.CRISP_EDGES;
			case "pixelated":
			case "-webkit-optimize-contrast": return IMAGE_RENDERING.PIXELATED;
			case "smooth":
			case "high-quality": return IMAGE_RENDERING.SMOOTH;
			default: return IMAGE_RENDERING.AUTO;
		}
	}
};
...
var testForeignObject = (document) => {
	...
	ctx.fillStyle = "red";
	ctx.fillRect(0, 0, size, size);
	return loadSerializedSVG$1(svg).then((img) => {
		ctx.drawImage(img, 0, 0);
...
var parseNodeTree = (context, node, parent, root) => {
	for (let childNode = node.firstChild, nextNode; childNode; childNode = nextNode) {
		...
		else if (isElementNode(childNode)) if (isSlotElement(childNode) && childNode.assignedNodes) childNode.assignedNodes().forEach((childNode) => parseNodeTree(context, childNode, parent, root));
		else {
...
var DocumentCloner = class {
	...
	toIFrame(ownerDocument, windowSize) {
		...
		const iframeLoad = iframeLoader(iframe).then(async () => {
			...
			if (typeof onclone === "function") return Promise.resolve().then(() => onclone(documentClone, referenceElement)).then(() => iframe);
			return iframe;
		});
		/**
		...
		* */
		...
		try {
			...
			if (!policy && trustedTypesFactory) policy = trustedTypesFactory.createPolicy("html2canvas-pro", { createHTML: (string) => string });
			if (policy) documentClone.write(policy.createHTML(rawHTML));
			else documentClone.write(rawHTML);
		} catch (_e) {
	...
	createElementClone(node) {
		...
		if (isCustomElement(clone) && !isSVGElementNode(clone)) return this.createCustomElementClone(clone);
		return clone;
	}
	...
	*/
	safeAppendClonedChild(target, child, copyStyles) {
		if (this.shouldCloneChild(child) && this.shouldCloneStyleElement(child)) target.appendChild(this.cloneNode(child, copyStyles));
	}
	/**
...
var copyCSSStyles = (style, target) => {
	for (let i = style.length - 1; i >= 0; i--) {
		...
		if (ignoredStyleProperties.indexOf(property) === -1 && !property.startsWith("--")) target.style.setProperty(property, style.getPropertyValue(property));
	}
	return target;
};
...
var parseStackTree = (parent, stackingContext, realStackingContext, listItems) => {
	parent.container.elements.forEach((child) => {
		const treatAsRealStackingContext = contains(child.flags, 4);
		const createsStackingContext = contains(child.flags, 2);
		const paintContainer = new ElementPaint(child, parent);
		if (contains(child.styles.display, 2048)) listItems.push(paintContainer);
		const listOwnerItems = contains(child.flags, 8) ? [] : listItems;
	...
	*/
	renderRadialGradient(container, backgroundImage, index) {
		...
		if (rx > 0 && ry > 0) {
			...
			if (rx !== ry) {
				const midX = container.bounds.left + .5 * container.bounds.width;
				const midY = container.bounds.top + .5 * container.bounds.height;
				const f = ry / rx;
				const invF = 1 / f;
				this.ctx.save();
	...
	*/
	formatPath(paths) {
		paths.forEach((point, index) => {
			...
			if (index === 0) this.ctx.moveTo(start.x, start.y);
			else this.ctx.lineTo(start.x, start.y);
			if (isBezierCurve(point)) this.ctx.bezierCurveTo(point.startControl.x, point.startControl.y, point.endControl.x, point.endControl.y, point.end.x, point.end.y);
		});
	}
};
...
var TextRenderer = class {
	...
	*/
	renderTextWithLetterSpacing(text, letterSpacing, baseline) {
		...
		else this.iterateLettersWithLetterSpacing(text, letterSpacing, baseline, (letter, x, y) => {
			...
			switch (paintOrderLayer) {
				...
				case 1:
					if (styles.webkitTextStrokeWidth && textBound.text.trim().length) {
						...
						if (styles.letterSpacing === 0) this.ctx.strokeText(textBound.text, textBound.bounds.left, textBound.bounds.top + styles.fontSize.number);
						else this.iterateLettersWithLetterSpacing(textBound, styles.letterSpacing, styles.fontSize.number, (letter, x, y) => this.ctx.strokeText(letter, x, y));
	...
	async renderTextNode(text, styles, containerBounds) {
		...
		if (styles.webkitLineClamp > 0 && (styles.display & 2) !== 0 && styles.overflowY === 1 && text.textBounds.length > 0) {
			...
			if (lines.length > maxLines) {
				for (let i = 0; i < maxLines - 1; i++) lines[i].forEach((textBound) => {
				...
				if (lastLine && lastLine.length > 0 && containerBounds) {
					...
					paintOrder.forEach((paintOrderLayer) => {
						switch (paintOrderLayer) {
							case 0:
								this.ctx.fillStyle = asString(styles.color);
								if (styles.letterSpacing === 0) this.ctx.fillText(truncatedText, firstBound.bounds.left, firstBound.bounds.top + styles.fontSize.number);
								else this.iterateLettersWithLetterSpacing(truncatedBounds, styles.letterSpacing, styles.fontSize.number, (letter, x, y) => this.ctx.fillText(letter, x, y));
								break;
							case 1:
								if (styles.webkitTextStrokeWidth && truncatedText.trim().length) {
									...
									this.ctx.lineWidth = styles.webkitTextStrokeWidth;
									this.ctx.lineJoin = typeof window !== "undefined" && !!window.chrome ? "miter" : "round";
									if (styles.letterSpacing === 0) this.ctx.strokeText(truncatedText, firstBound.bounds.left, firstBound.bounds.top + styles.fontSize.number);
									else this.iterateLettersWithLetterSpacing(truncatedBounds, styles.letterSpacing, styles.fontSize.number, (letter, x, y) => this.ctx.strokeText(letter, x, y));
									this.ctx.strokeStyle = "";
									this.ctx.lineWidth = 0;
									this.ctx.lineJoin = "miter";
								}
		...
		if (shouldApplyEllipsis) {
			...
			if (text.textBounds.every((tb) => Math.abs(tb.bounds.top - firstTop) < lineHeight * .5)) {
			...
			paintOrder.forEach((paintOrderLayer) => {
				switch (paintOrderLayer) {
					case 0: {
						...
						if (styles.textDecorationLine.length) this.renderTextDecoration(text.bounds, styles);
						break;
					}
					case 1:
						if (styles.webkitTextStrokeWidth && text.text.trim().length) {
							...
							if (styles.letterSpacing === 0) this.ctx.strokeText(text.text, text.bounds.left, text.bounds.top + baseline);
							else this.iterateLettersWithLetterSpacing(text, styles.letterSpacing, baseline, (letter, x, y) => this.ctx.strokeText(letter, x, y));
							this.ctx.strokeStyle = "";
							this.ctx.lineWidth = 0;
							this.ctx.lineJoin = "miter";
						}
...
var MASK_OFFSET = 1e4;
var CanvasRenderer = class CanvasRenderer extends Renderer {
	constructor(context, options) {
	...
	*/
	...
	async renderNodeContent(paint) {
		...
		if (container instanceof IFrameElementContainer && container.tree) {
			const canvas = await new CanvasRenderer(this.context, {
	...
	async renderNodeBackgroundAndBorders(paint) {
		...
		for (const border of borders) {
			if (border.style !== 0 && !isTransparent(border.color) && border.width > 0) if (border.style === 2) await this.borderRenderer.renderDashedDottedBorder(border.color, border.width, side, paint.curves, 2);
...
var Cache = class {
	...
	async _addImageInternal(src) {
		...
		const timeoutPromise = new Promise((_, reject) => {
			setTimeout(() => {
				reject(/* @__PURE__ */ new Error(`Image load timeout after ${timeoutMs}ms: ${src}`));
			}, timeoutMs);
		});
	...
	*/
	set(key, value) {
		...
		if (this._cache.size >= this.maxSize) this.evictLRU();
		this._cache.set(key, {
			value,
	...
	*/
	evictLRU() {
		...
		for (const [key, entry] of this._cache.entries()) if (entry.lastAccessed < oldestTime) {
	...
	*/
	...
	async loadImage(key) {
		...
		return await new Promise((resolve, reject) => {
			...
			if (this._options.imageTimeout > 0) setTimeout(() => reject(`Timed out (${this._options.imageTimeout}ms) loading image`), this._options.imageTimeout);
		});
	}
	has(key) {
...
var Html2CanvasConfig = class Html2CanvasConfig {
	constructor(options = {}) {
		...
		if (!this.window) throw new Error("Window object is required but not available");
...
var setCspNonce = (nonce) => {
	...
	if (typeof window !== "undefined") setDefaultConfig(new Html2CanvasConfig({
		window,
		cspNonce: nonce
	}));
};
