import { c as prepareBuiltInToolInvocation } from "./BuiltInToolRegistry-DDqJSJVu.mjs";
...
import { n as acquireProjectOwnership, t as OwnershipHelperError } from "./OwnershipHelper-SR8YuqaD.mjs";
...
async function runPathRoutedInvocation$1(options, invokeClosedProjectTool) {
	...
	let prepared;
	try {
		prepared = prepareBuiltInToolInvocation(options.toolName, options.input, (capabilities) => canClosedProjectProvideCapabilities(capabilities) || canOpenProjectProvideCapabilities(capabilities));
	} catch {
		return {
			ok: false,
			error: {
...
function invokeClosedProjectToolInProduction(options) {
	return runClosedProjectRuntime(async () => {
		const { invokeClosedProjectTool, loadPrecompiledClosedProjectModule } = await import("./ClosedProjectInvocation-t5_XPuG6.mjs");
