...
import { a as Entity, et as __decorate, tt as __decorateMetadata } from "./ProjectUpgrader-C2CEsQ4r.mjs";
import { r as service, t as Project } from "./Project-BFDRwv_G.mjs";
import { t as Edge } from "./Edge-BYuHSHju.mjs";
import { t as Section } from "./Section-B8z7ZNXn.mjs";
import { t as MultiTargetUndirectedEdge } from "./MutiTargetUndirectedEdge-CvYAY55D.mjs";
//#region src/core/stage/stageManager/basicMethods/SectionMethods.tsx
/**
...
*/
var SectionMethods = class SectionMethods {
	project;
	bigTitleCacheFrame = -1;
	bigTitleActiveCache = /* @__PURE__ */ new Map();
	bigTitleCoveringAncestorCache = /* @__PURE__ */ new Map();
	prepareBigTitleFrameCache() {
		if (this.bigTitleCacheFrame === this.project.renderer.frameIndex) return;
	...
	* @complexity O(1)
	*/
	getFatherSections(entity) {
		if (entity.parentSection) return [entity.parentSection];
		return [];
	}
	/**
	...
	*/
	isObjectBeLockedBySection(object) {
		if (object instanceof Entity) {
			if (object instanceof Section && object.locked) return true;
			return object.nearestLockedAncestorSection !== null;
		} else if (object instanceof Edge) return object.source.nearestLockedAncestorSection !== null || object.target.nearestLockedAncestorSection !== null;
		else if (object instanceof MultiTargetUndirectedEdge) {
			for (const entity of object.associationList) if (entity.nearestLockedAncestorSection !== null) return true;
			return false;
		}
		return false;
	}
	...
	*/
	getSectionsByInnerLocation(location) {
		const result = [];
		for (const rootSection of this.project.stageManager.getRootSections()) result.push(...this.getDeepestSectionsAtLocation(rootSection, location));
	...
	*/
	getInnermostSectionByLocation(location) {
	...
	* @complexity O(1)
	*/
	isSectionBigTitleActive(section) {
		...
		const active = Math.max(section.rectangle.size.x, section.rectangle.size.y) < Math.max(this.project.renderer.w, this.project.renderer.h) / this.project.camera.currentScale * Settings.sectionBigTitleThresholdRatio && this.project.camera.currentScale <= Settings.sectionBigTitleCameraScaleThreshold;
	...
	* @complexity O(d)，d 为实体祖先链深度
	*/
	getBigTitleCoveringAncestorSection(entity) {
		this.prepareBigTitleFrameCache();
		if (this.bigTitleCoveringAncestorCache.has(entity)) return this.bigTitleCoveringAncestorCache.get(entity) ?? null;
		let current = entity.parentSection;
		while (current) {
	...
	* @complexity O(d)，d 为实体祖先链深度
	*/
	isEntityHiddenByBigTitleSection(entity) {
		if (!Settings.hideSectionContentsWhenBigTitleActive) return false;
	...
	* @complexity O(d)，d 为实体祖先链深度
	*/
	isEntityCoveredByBigTitleSection(entity) {
		return this.getBigTitleCoveringAncestorSection(entity) !== null;
	}
	/**
	...
	* @complexity O(d)，d 为实体祖先链深度
	*/
	getBigTitleCoveringAncestorSections(entity) {
		const result = [];
		let current = entity.parentSection;
		while (current) {
			if (this.isSectionBigTitleActive(current)) result.push(current);
			current = current.parentSection;
		}
		return result;
	}
	...
	*/
	isAssociationHiddenByBigTitleSection(association) {
		if (!Settings.hideSectionContentsWhenBigTitleActive) return false;
		const entityEndpoints = association.associationList.filter((stageObject) => stageObject instanceof Entity);
		if (entityEndpoints.length === 0) return false;
		...
		if (ancestorSets.some((set) => set.size === 0)) return false;
		const [firstSet, ...restSets] = ancestorSets;
		return [...firstSet].some((uuid) => restSets.every((set) => set.has(uuid)));
	}
	/**
	...
	* @complexity O(k·d)，k 为端点数，d 为祖先链深度
	*/
	isAssociationCoveredByBigTitleSection(association) {
	...
	* @complexity O(d)，d 为实体祖先链深度
	*/
	getOutermostLockedAncestorSection(entity) {
	...
	*/
	shallowerNotSectionEntities(entities) {
		const sections = entities.filter((entity) => entity instanceof Section);
		const sectionUUIDSet = new Set(sections.map((section) => section.uuid));
		const result = [];
		for (const entity of entities) {
			if (entity instanceof Section) continue;
			let current = entity.parentSection;
			...
			if (!isAnyChild) result.push(entity);
		}
	...
	* @complexity O(d)，d 为实体祖先链深度
	*/
	isEntityInSection(entity, section) {
		let current = entity.parentSection;
		while (current) {
	...
	*/
	getSectionMaxDeep(section) {
		const visited = /* @__PURE__ */ new Set();
		const dfs = (node, deep = 1) => {
			if (visited.has(node.uuid)) return deep;
			visited.add(node.uuid);
			for (const child of node.children) if (child instanceof Section) deep = Math.max(deep, dfs(child, deep + 1));
			return deep;
		};
		return dfs(section);
	}
	...
	*/
	getAllEntitiesInSelectedSectionsOrEntities(selectedEntities) {
		const entityUUIDSet = /* @__PURE__ */ new Set();
		const dfs = (currentEntity) => {
			if (entityUUIDSet.has(currentEntity.uuid)) return;
			if (currentEntity instanceof Section) for (const child of currentEntity.children) dfs(child);
			entityUUIDSet.add(currentEntity.uuid);
		};
		for (const entity of selectedEntities) dfs(entity);
	...
	*/
	getDeepestSectionsAtLocation(section, location) {
		if (section.isCollapsed || section.isHiddenBySectionCollapse) return [];
		if (!section.collisionBox.getRectangle().isPointIn(location)) return [];
		const deeperSections = [];
		for (const child of section.children) if (child instanceof Section) deeperSections.push(...this.getDeepestSectionsAtLocation(child, location));
		if (deeperSections.length > 0) return deeperSections;
		return [section];
	}
};
SectionMethods = __decorate([service("sectionMethods"), __decorateMetadata("design:paramtypes", [typeof Project === "undefined" ? Object : Project])], SectionMethods);
//#endregion
export { SectionMethods as t };
