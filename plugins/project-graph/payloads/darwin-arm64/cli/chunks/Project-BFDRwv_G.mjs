...
import { i as createLiveProjectBuiltInToolRuntimeHost, r as classifyBuiltInToolException, s as invokeBuiltInTool } from "./BuiltInToolRegistry-BSSuJBGq.mjs";
...
//#endregion
//#region ../node_modules/.pnpm/@radix-ui+react-primitive@2.1.4_@types+react-dom@19.2.3_@types+react@19.2.14__@types+re_79f9cc29726bbcca5df2cac469f5e931/node_modules/@radix-ui/react-primitive/dist/index.mjs
var Primitive = [
	"a",
	"button",
	"div",
...
//#endregion
//#region ../node_modules/.pnpm/@radix-ui+react-separator@1.1.8_@types+react-dom@19.2.3_@types+react@19.2.14__@types+re_35c53c555750bc91a5b469d5f137708e/node_modules/@radix-ui/react-separator/dist/index.mjs
var NAME = "Separator";
...
var Tab = class extends import_react.Component {
	...
	*/
	...
	async dispose() {
		this.pause();
		const cleanupTasks = Array.from(this.services.values(), (service) => Promise.resolve().then(() => service.dispose?.()));
...
//#endregion
//#region ../node_modules/.pnpm/jotai@2.20.1_@babel+core@7.29.0_@babel+template@7.28.6_@types+react@19.2.14_react@19.2.7/node_modules/jotai/esm/vanilla/internals.mjs
...
//#endregion
//#region ../node_modules/.pnpm/jotai@2.20.1_@babel+core@7.29.0_@babel+template@7.28.6_@types+react@19.2.14_react@19.2.7/node_modules/jotai/esm/vanilla.mjs
var keyCount = 0;
...
//#endregion
//#region ../node_modules/.pnpm/jotai@2.20.1_@babel+core@7.29.0_@babel+template@7.28.6_@types+react@19.2.14_react@19.2.7/node_modules/jotai/esm/react.mjs
...
function projectIdentity(project) {
	...
	const projectUri = project.canonicalProjectPath ? URI.file(project.canonicalProjectPath).toString() : legacyProjectUri;
	return {
		projectUri,
...
(function(_AIProjectReferenceStore) {
	function load(project) {
		return invoke("load_project_reference_snapshot", projectIdentity(project));
	}
	_AIProjectReferenceStore.load = load;
	function save(project, references) {
		const { projectUri } = projectIdentity(project);
		const previousWrite = writeQueues.get(projectUri) ?? Promise.resolve();
		const persist = () => invoke("save_project_reference_snapshot", {
			projectUri,
			references
		});
		...
		write.then(removeCompletedWrite, removeCompletedWrite);
		return write;
	}
...
var ProjectOwnershipLifecycle = class {
	project;
	ownership;
	runtimeHost;
	constructor(project) {
	...
	attach(ownership) {
		if (this.ownership && this.ownership !== ownership) throw new Error("Project ownership is already attached");
		this.ownership = ownership;
	}
	activate() {
		if (this.ownership) this.runtimeHost ??= registerOpenProjectRuntimeHost(this.project);
	}
	async saveAs(targetUri, write) {
		...
		try {
			...
			if (createdRuntimeHost) {
				...
				this.runtimeHost = void 0;
				try {
					await runtimeHost?.dispose();
				} catch (rollbackError) {
					rollbackErrors.push(rollbackError);
				}
			} else if (previousRuntimeHost && previousOwnership) try {
			...
			if (rollbackErrors.length > 0) throw new AggregateError([error, ...rollbackErrors], "Project ownership transition failed", { cause: error });
			throw error;
		}
		if (previousOwnership) await releaseProjectOwnershipWithRetry(previousOwnership);
	}
	async dispose(disposeProject) {
		...
		try {
			...
			if (ownership) {
				...
				if (this.ownership === ownership) this.ownership = void 0;
			}
		}
...
var Project = class Project extends Tab {
	...
	*/
	...
	_projectState = 2;
	_isSaving = false;
	projectOwnership = new ProjectOwnershipLifecycle(this);
	stage = [];
	tags = [];
	/**
	* string：UUID
